...
- `*.png` — visualizations, including listener-state, surface-evidence heatmap, and reading-map plots for human review
...
```bash
galdr assemble <track-name> [--template arc|arc-family|first|none] [--lens default|sound|dancefloor|structure|meaning|lyrics-study|classical|ritual] [--mode blind|lyrics|context|full]
```
