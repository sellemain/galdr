...
|---|---|---|
| `attention_grip` | Attention grip | the music has hold of attention |
| `pattern_integrity` | Pattern integrity | the music keeps carried time intact |
| `pulse_steadiness` | Pulse steadiness | the measured pulse is stable |
| `body_grip` | Body grip | the pulse takes the body, not just the ear |
| `physical_hold` | Physical hold | weight, drag, suspension, or room-pressure arrives |
| `weight_arc` | Weight arc | where the track puts macro weight over time |
| `pressure_motion` | Pressure motion | pressure builds, releases, sustains, or empties |
| `texture_weight` | Texture weight | weight sits in sustained tone or percussive attack |
| `harmonic_pull` | Harmonic pull | the tonal field moves, pulls, or refuses to settle |
| `harmonic_color_motion` | Harmonic color motion | harmonic color changes over time |
| `tonal_anchor` | Tonal anchor | the local key center holds or loosens |
| `pitch_grid_alignment` | Pitch-grid alignment | pitch sits inside or outside equal-tempered grid space |
...
| `foreground_line_evidence` | Foreground line evidence | a foreground pitched line is trackable |
...
Implementation evidence: rolling beat regularity and beat density over an 8-second window. Regular intervals with enough beat evidence produce high attention grip; missing, sparse, or irregular beat evidence lowers it.

Not a quality judgment. High attention grip measures the grip, not the value of what's gripping. Holds through short silences (<~2s); falls and requires re-entry after sustained silence or structural breakdown.
...
Do not write "LUFS rises from -27 to -20" in experience prose. That belongs in debugging or regression notes. A high-attention grip track with negative pressure motion is exhaling — resonance is still locked. Negative pressure motion is not failure.

---

### `texture_weight`
Display name: **Texture weight**. Where the track's weight sits between sustained harmonic sound and percussive impact. Negative = harmonic/vocal dominant. Positive = percussive dominant.
...
### `pattern_integrity`
How reliably the music keeps its pattern intact. `pattern_integrity = 1.0 - disruption`. High = steady pulse, stable texture, and few structural surprises. Low = irregular pulse, sudden break, or sharp textural reorientation.
...
High pattern_integrity is not rigidity. In ritual and ceremonial music, extreme regularity is intentional — it allows the listener to stop tracking the beat and go somewhere else. High mean_pattern_integrity across a track doesn't mean boring; it may mean the music is consistent enough to allow depth.
...
Low pitch_grid_alignment is not out-of-tune. Folk traditions, overtone singing, and ritual music frequently sit here by design. Often co-occurs with deeply negative texture_weight.
...
Multiple silences clustering at a track's end, getting progressively longer and deeper, are a specific gesture — the music departing in stages. If attention grip re-establishes after each silence (returning to `0.9+`), the listener is still held even as the music withdraws. Departure and grip happening simultaneously.

---

### `weight_arc`
Display name: **Weight arc**. The track divided into segments with mean and peak energy per segment — the macro shape of where the music puts its weight. Independent from attention grip: low-energy sections can maintain full resonance.
...
|---|---|---|
| `attention_grip` | Beat regularity × beat density in rolling windows | How strongly attention is carried forward |
| `pressure_motion` / `pressure_state` | Short-term LUFS movement | Whether pressure builds, releases, sustains, or empties |
| `pattern_integrity` | `1.0 - disruption`; disruption = beat + spectral + energy expectation failures | How intact the musical pattern feels |
| `texture_weight` / Texture weight | Harmonic/percussive separated energy | Whether weight sits in sustained tone or attack |
| `pitch_grid_alignment` | Chroma concentration in equal-tempered pitch classes | How centered or outside-the-grid the pitch world feels |
...
| `harmonic_color_motion` | Cosine distance between adjacent chroma frames | How quickly harmonic color changes |
...
| `foreground_line_evidence` | pYIN voiced probability over time | How strongly a foreground pitched line is present |
| `silences` | dB-floor intervals plus recovery attention grip | Where the structure truly empties and whether it re-locks |
...
Summary values (`mean_attention_grip`, `mean_pattern_integrity`, `mean_harmonic_pull`, and similar `mean_*` fields) describe the track's overall tendency. They are useful for comparison, cataloging, and first-pass orientation. They are not the listening experience by themselves. A track can have a high mean value because it stays there for six minutes, or because it repeatedly surges and collapses. The mean tells you where to look; the stream tells you what happened.

Stream values are time-indexed listener-state samples. Read them as motion:

- **rising `attention_grip`** → the listener is being pulled into the structure
- **stable high `attention_grip`** → attention is locked and being carried
- **falling `attention_grip`** → the structure is losing its grip or entering a real gap
- **high `pattern_integrity`** → expectation is being honored; the pattern is intact
- **low `pattern_integrity`** → rhythm, texture, or energy has violated expectation
- **positive `pressure_motion`** → pressure is coming forward
- **negative `pressure_motion`** → pressure is releasing
- **negative `texture_weight`** → sustained tone, voice, drone, or harmony dominates
- **positive `texture_weight`** → strike, groove, attack, or percussion dominates

Events are narrative anchors derived from the stream. Macro events (`attention_grip_arrives`, `body_lock_arrives`, `surface_hardens`, `weight_arrives`) describe major state changes. Phrase events (`phrase_lift`, `phrase_drop`, `ornamental_flash`) describe smaller local gestures inside a stable state. Events are not a checklist to recite. They are signposts for where prose should slow down and listen more closely.
...
- High `attention_grip` + high `pattern_integrity` + steady `pressure_motion` = the track has the listener and is holding them without strain.
- High `attention_grip` + low `pattern_integrity` = the listener is still gripped, but the surface is unstable or disruptive.
- Negative `texture_weight` + high `overtone_fit` = sustained pitched material is carrying the experience through resonance rather than impact.
- Falling `pressure_motion` + high `attention_grip` = the pressure is releasing, but attention has not let go.
- Silence + fast attention grip recovery = the music empties briefly, then reclaims the listener.

Treat field names as implementation labels. In public prose, do not write that a track has “high `mean_foreground_line_evidence`” or “a `phrase_drop` event.” Write what those fields are evidence for: a foreground line appears; the phrase falls away; the body of the sound arrives; the pattern breaks.
...
**Mechanical readout:**
> "attention grip: 0.974 (high), pattern_integrity: 0.964 (high), texture_weight: -0.518 (harmonic dominant), pressure_state: sustaining"
...
7 minutes 6 seconds. The track opens with 7 seconds of true silence. When sound arrives, attention grip stays at `0.000` for 30 seconds — typical tracks lock in 4–7 seconds. Helvegen takes thirty. Then between t=30s and t=60s, resonance locks: attention grip jumps from `0.000` to `0.974` and holds between 0.96–0.98 for five and a half minutes.

At the end, seven silences cluster. Each longer and deeper: `1.0s → 1.2s → 0.5s → 0.5s → 1.5s → 1.5s → 9.5s`, descending from -60dB to -80dB. After each silence, the voice returns. Resonance re-establishes — attention grip returns above 0.93 each time. The listener keeps re-engaging because the music keeps coming back at full conviction. Until the seventh silence, when the voice doesn't return.
