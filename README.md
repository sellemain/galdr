...
For models that accept audio, Galdr can request an independent Inner Ear listen and later reconcile that witness packet with its computed analysis. An optional Gemini adapter provides an explicit end-to-end command while normal analysis stays local and credential-free. See [Inner Ear witness packets](docs/INNER-EAR.md).
