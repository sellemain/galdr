...
For models that accept audio, Galdr can assemble a provider-neutral Inner Ear discovery prompt and later include the resulting witness packet as bounded evidence. An optional Gemini adapter provides an explicit end-to-end command while normal analysis stays local and credential-free. See [Inner Ear witness packets](docs/INNER-EAR.md).
