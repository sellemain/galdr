...
Use only audible musical evidence and its audio clock. A text-free vocal timing packet may be supplied as private alignment evidence: use its windows to locate phrase entrances, continuations, gaps, and endings more precisely. No lyric text or semantic content belongs in this lens. Do not quote, paraphrase, summarize, or interpret words, including short vocal syllables and refrain fragments. Do not use song meaning, biography, genre history, genre labels, or visual context. A voice may be described as grain, breath, register, distance, attack, mass, phrasing, or pressure, but not interpreted semantically. Galdr events and the optional hearing stream may locate candidate moments; they do not require a cue and their internal labels are not public prose. If a hearing-stream vocal-state claim conflicts with a source-bound vocal window, the vocal window wins.
...
Treat hearing-stream timestamps as observation samples and useful neighborhoods to inspect, not automatically as event onsets. For a cue about an entrance, cutoff, or forceful transition, return to the exact recording around that neighborhood and place `musicalAnchorMs` where the change becomes clearly audible. A source-bound reviewed event map or a focused local-window ear pass can provide that confirmation. When only the sampled state is supported, write about the state already present there or place the cue after recognition instead of turning the sample time into a sharper claim than the evidence carries.
...
Audit the spaces between cues after drafting. For any gap around 20 seconds or longer, check whether a concrete sound or vocal-surface change was missed; add speech only when it improves the listening. A gap around 40 seconds or longer requires an explicit review decision, not an automatic cue. Add orientation when a useful hearing would otherwise be lost; preserve uninterrupted music when speech would add little, and record that private decision in the adjacent grounding. Do not let minute-scale gaps happen accidentally. This is a continuity audit, not a density quota.
