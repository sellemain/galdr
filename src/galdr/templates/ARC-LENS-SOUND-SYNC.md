...
Before returning, silently check every cue: Is the named event already audible? Is the claim local and source-matched? Does this sentence add more than it masks? Is the gap before the next cue intentional and compliant with the gap audit? Could any cue be removed without losing a useful hearing? Accuracy beats coverage.
