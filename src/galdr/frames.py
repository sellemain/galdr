...
def _score_event(ev: dict) -> float:
    ...
    if ev["type"] in ("attention_drop", "attention_gain"):
...
def _event_label(ev: dict) -> str:
    ...
    if ev["type"] == "attention_drop":
        frm = ev.get("from", 0.0)
        to = ev.get("to", 0.0)
        return f"attention drop ({frm:.3f} → {to:.3f})"
    if ev["type"] == "attention_gain":
        frm = ev.get("from", 0.0)
        to = ev.get("to", 0.0)
        return f"attention gain ({frm:.3f} → {to:.3f})"
